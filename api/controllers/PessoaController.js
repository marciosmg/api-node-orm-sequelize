...
class PessoaController {
  ...
  static async apagaMatricula(req, res) {
    ...
    try {
      ...
      return res.status(200).json({ mensagem: `Matrícula ${matriculaId} foi deletada!`})
    } catch(error) {
      return res.status(500).json(error.message)
    }
  }
  ...
  static async pegaMatriculasPorTurma(req, res) {
    ...
    try {
      const todasAsMatriculas = await database.Matriculas.findAndCountAll({ where: { 
        ...
        status: 'confirmado',
        limit: 20,
        order: [['estudante_id', 'ASC']] // 'ASC ascendente - oposto de 'DESC' descendente
      } })
  ...
  static async cancelaPessoa(req, res) {
    ...
    try {
      database.sequelize.transaction(async transacao => {
        ...
        return res.status(200).json({ message: `Pessoa ${estudanteId} e suas matrículas foram canceladas!`})
      })
    } catch(error) {
      return res.status(500).json(error.message)
    }
